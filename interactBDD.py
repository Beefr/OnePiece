...
class InteractBDD(Static):
	...
	def pirateTXT(elem, conn, cur, type):
		...
		txt=r'{'+ '"type": "{}", "name": "{}", "level": "{}", "qualite": "{}", "fruit": "{}", "stats": "{}", "availableToFight": "True", "mort": "False"'.format(type, piratesName, str(level), str(qualite), fruitsTXT, str(StatsPirate.generateStats(level, qualite, power)) ) +'}'
		return txt
